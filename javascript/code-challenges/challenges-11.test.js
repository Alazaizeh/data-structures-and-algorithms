...
Write a function named transformToLis that, given an object, returns an array of the key value
 pairs as html list items.
...
uses either filter, map, or reduce to count the amount of times the integer is present in
 the array of arrays.
...
const count = (target, input) =>
  input.reduce(
    (total, arr) =>
    (total += arr.reduce(
      (acc, ele) => (ele == target ? (acc += 1) : (acc += 0)),
      0
    )),
    0
  );
...
const totalSum = (input) =>
  input.reduce(
    (total, arr) => (total += arr.reduce((acc, ele) => (acc += ele), 0)),
    0
  );
...
uses any combination of filter, map and reduce to return the name of the character
 who is the shortest in height.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => data.reduce((acc, obj) => {
  if (parseInt(obj.height) < acc.height) {
    acc.height = obj.height;
    acc.starName = obj.name;
  }
  return acc;
}, { height: Number.MAX_SAFE_INTEGER, starName: "" }).starName;
/* ------------------------------------------------------------------------------------------------
TESTS
