...
const addValues = (arr) => arr.reduce((acc, ele) => (acc += ele), 0);
...
const addPurchases = (arr) =>
  arr.reduce((acc, ele) => (acc += ele.purchasePrice), 0);
...
const countNumberOfChildren = (arr) =>
  arr.reduce(
    (acc, ele) => (acc += ele["children"] ? ele.children.length : 0),
    0
  );
...
const calculateAverage = (arr) => {
  let avg = arr.reduce(
    (acc, ele) =>
      Object.assign(acc, { count: acc.count + 1, sum: acc.sum + ele }),
    { count: 0, sum: 0 }
  );
...
const countPrimeNumbers = (arr) =>
  arr.reduce((acc, ele) => (isPrime(ele) ? ++acc : acc), 0);
...
const extractStat = (statName, arr) => {
  return arr.reduce(
    (acc, ele) => (ele.stat.name === statName ? (acc = ele) : acc),
    null
  );
};
...
const extractChildren = (arr) =>
  arr
    .filter((ele) => ele.name.includes("a"))
    .reduce((acc, ele) => {
      if (ele.children) {
